...
namespace ProductIdentification.Data.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            return await _context.Products.Where(x => x.Category.Id == category.Id).ToListAsync();
        }

        public async Task<List<Product>> GetAll(SubCategory subcategory)
        {
            return await _context.Products.Where(x => x.SubCategory.Id == subcategory.Id).ToListAsync();
        }
        ...
        {
            return await _context.Products.Where(x => x.SubCategory.Id == subcategoryId).ToListAsync();
        }
    }
}
